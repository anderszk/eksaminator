...
export default function TreningIndexPage() {
  ...
  useEffect(() => {
    api.documents.list()
      .then((list) => {
        setDocs(list);
        if (list.length > 0) router.replace(`/trening/new?docId=${list[0].id}`);
      })
      .catch(() => setDocs([]));
  }, [router]);
  ...
  return (
    <main className="page" style={{ maxWidth: 560, textAlign: "center", paddingTop: 80 }}>
      ...
      <p style={{ color: "var(--ink-muted)", marginBottom: 28 }}>Last opp masteroppgaven din før du kan starte en treningsøkt.</p>
